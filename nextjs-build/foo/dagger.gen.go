...
import (
	...
	"nextjs-build/internal/querybuilder"
)
...
func invoke(ctx context.Context, parentJSON []byte, parentName string, fnName string, inputArgs map[string][]byte) (any, error) {
	...
	case "Directory":
		...
		case "NextJSBuild":
			...
			return (*Directory).NextJSBuild(&parent), nil
		default:
	...
	case "NextjsBuild":
		...
		case "Build":
			...
			return (*NextjsBuild).Build(&parent, source), nil
		default:
			return nil, fmt.Errorf("unknown function %s", fnName)
		}
	case "":
		return dag.CurrentModule().
			WithObject(
				dag.TypeDef().WithObject("NextjsBuild").
					WithFunction(
						dag.NewFunction("Build",
							dag.TypeDef().WithObject("Container")).
							WithDescription("Use a directory to build a NextJS app.\n").
							WithArg("source", dag.TypeDef().WithObject("Directory")))).
			WithObject(
				dag.TypeDef().WithObject("Directory").
					WithFunction(
						dag.NewFunction("NextJSBuild",
